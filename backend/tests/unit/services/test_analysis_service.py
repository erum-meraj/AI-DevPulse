...
async def make_clustered_article(
    article_repo, cluster_id, index: int, score: int
) -> Article:
...
@pytest.mark.asyncio
async def test_analyze_cluster_persists_fields_statuses_and_topics(
    async_session,
) -> None:
    ...
    analysis = await AnalysisService(async_session, provider).analyze_cluster(
        cluster, articles
    )
    ...
    topics_after_rerun = await TopicRepository(async_session).list_for_article(
        articles[0].id
    )
