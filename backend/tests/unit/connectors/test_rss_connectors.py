...
from backend.app.connectors.anthropic_rss import AnthropicRSSConnector
from backend.app.connectors.huggingface_rss import HuggingFaceRSSConnector
from backend.app.connectors.openai_rss import OpenAIRSSConnector
...
async def test_rss_connector_falls_back_to_summary_content() -> None:
    respx.get(OPENAI_RSS_FEED_URL).mock(
        return_value=Response(200, text=RSS_FEED_WITH_SUMMARY_ONLY)
    )
