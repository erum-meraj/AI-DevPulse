...
class EmbeddingService:
    ...
    async def embed_articles(self, articles: Sequence[Article]) -> dict[str, int]:
        ...
        for start in range(0, len(articles), EMBEDDING_BATCH_SIZE):
            ...
            batch_processed, batch_failed = await self._embed_batch_with_isolation(
                batch
            )
    ...
    async def _embed_articles_one_by_one(
        self, articles: list[Article]
    ) -> tuple[int, int]:
        processed = 0
        failed = 0
        for article in articles:
            try:
                embedding = await self.ai_provider.embed(
                    [self._build_embedding_text(article)]
                )
            except Exception:
                failed += 1
                continue
