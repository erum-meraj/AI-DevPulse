...
class AnalysisService:
        ...
        for article in member_articles:
            ...
            for existing_topic in existing_topics:
                if existing_topic.name not in topics:
                    await self.topic_repository.remove_article_topic(
                        article.id, existing_topic.id
                    )
            for topic_name in topics:
                topic = await self.topic_repository.get_or_create(topic_name)
                await self.topic_repository.add_article_topic_if_missing(
                    article.id, topic.id
                )
    ...
    def build_prompt(self, member_articles: list[Article]) -> str:
        ...
        template = "\n\n".join(templates)
        selected = sorted(
            member_articles, key=lambda article: article.score or 0, reverse=True
        )[:ANALYSIS_MAX_ARTICLES]
