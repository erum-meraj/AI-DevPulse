...
class ClusteringService:
    ...
    async def assign_cluster(self, article: Article) -> bool:
        ...
        nearest_cluster, similarity = await self.cluster_repository.find_nearest_by_embedding(
            article.embedding
        )
    ...
    async def _update_existing_cluster(self, cluster: StoryCluster) -> None:
        ...
        cluster.centroid_embedding = self.compute_centroid(
            [member.embedding for member in member_articles if member.embedding is not None]
        )
