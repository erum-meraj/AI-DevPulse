...
class RankingService:
    ...
    async def rank_cluster(self, cluster: StoryCluster, now: datetime | None = None) -> float:
        ...
        importance = self.calculate_importance(members, trend_score_for_cluster(trends), now)
    ...
    def calculate_importance(
        ...
        popularity_score = min(
            sum(article.score or 0 for article in members) / POPULARITY_NORMALIZATION_CAP,
            1.0,
        )
        current_time = now or datetime.now(UTC)
        most_recent = max(article.published_at for article in members)
        hours_since_published = max(0.0, (current_time - most_recent).total_seconds() / 3600)
