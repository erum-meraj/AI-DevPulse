...
async def test_dashboard_applies_date_and_highlight_caps(
    ...
    assert [cluster.importance for cluster in dashboard.top_clusters] == [100.0, 99.0, 98.0]
    assert len(dashboard.trend_highlights) == 5
    assert all(trend.status in {"exploding", "rising"} for trend in dashboard.trend_highlights)
